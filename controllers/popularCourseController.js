...
const createPopularCourse = async(req, res) => {
    const {coverImage, courseTitle,courseDetail,instructorImage,updateDate,coursePrice} = req.body

    try {
        const course = await popularCourseSchema.create({coverImage, courseTitle,courseDetail,instructorImage,updateDate,coursePrice});
...
const updatePopularCourse = async(req, res) => {
    ...
    try {
        const course = await popularCourseSchema.findByIdAndUpdate({_id: id}, {coverImage, courseTitle,courseDetail,instructorImage,updateDate,coursePrice}, {returnOriginal: false})
        res.status(200).json(course)
    } catch (error) {
        res.status(401).json({error: error.message})
    }
